...
class BGRL(nn.Module):
    ...
    def __init__(self, layer_config, pred_hid, dropout=0.0, moving_average_decay=0.99, epochs=1000,encoder_type='gcn', **kwargs):
        super().__init__()
        self._encoder_type = encoder_type

        if encoder_type in ("gat", "dotgat"):
            nhead=4
            assert len(layer_config) >= 2 and layer_config[1] % nhead == 0
            enc_num_hidden = layer_config[1] // nhead
            enc_nhead = nhead
        else:
            enc_num_hidden = layer_config[1]
            enc_nhead = 1
        self.student_encoder = setup_module(
            m_type=encoder_type,
            enc_dec="encoding",
            in_dim=layer_config[0],
            num_hidden=enc_num_hidden,
            out_dim=enc_num_hidden,
            num_layers=len(layer_config)-1,
            nhead=enc_nhead,
            nhead_out=enc_nhead,
            concat_out=True,
            activation='relu',
    ...
    def forward(self, graph1,x1,graph2,x2):
        v1_student = self.student_encoder(graph1,x1)
        v2_student = self.student_encoder(graph2,x2)

        v1_pred = self.student_predictor(v1_student)
        v2_pred = self.student_predictor(v2_student)

        with torch.no_grad():
            v1_teacher = self.teacher_encoder(graph1,x1)
            v2_teacher = self.teacher_encoder(graph2,x2)
