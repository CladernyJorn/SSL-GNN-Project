    ...
    def __init__(self,
        ...
        if num_layers == 1:
            self.gat_layers.append(DotGatConv(
                ...
                feat_drop, attn_drop, last_residual, norm=last_norm, concat_out=concat_out,allow_zero_degree=allow_zero_degree))
        else:
            # input projection (no residual)
            self.gat_layers.append(DotGatConv(
                in_dim, num_hidden, nhead,
                feat_drop, attn_drop, residual, create_activation(activation), norm=norm, concat_out=concat_out,allow_zero_in_degree=allow_zero_degree))
            # hidden layers
